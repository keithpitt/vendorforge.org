  ...
  context 'setting a vendor spec' do
    ...
    let(:existing_vendor_spec) {
        { :authors =>"keithpitt", :files => ["DKBenchmark.h", "DKBenchmark.m"],
          :homepage =>"http://www.keithpitt.com", :github => "https://github.com/keithpitt/DKBenchmark",
    ...
    let(:new_vendor_spec) {
        { "authors"=> ["keithpitt", "mariovisic" ], "files"=>["DKBenchmark.h", "DKBenchmark.m"],
          "homepage"=>"http://www.mariovisic.com", "github"=>"https://github.com/mariovisic/DKBenchmark",
    ...
    context 'with an existing vendor' do
      ...
      it 'should use an existing vendor owned by the user if it exists' do
        version = Version.new(:user => user, :vendor_spec => new_vendor_spec)
        version.save
      ...
      it "should add an error if the user doesn't own the existing vendor" do
        version = Version.new(:user => other_user, :vendor_spec => new_vendor_spec)
      ...
      it 'should overwrite atributes on the existing vendor' do
        ...
        version.vendor.github.should == "https://github.com/mariovisic/DKBenchmark"
      ...
      it 'should increase the version count on the verndors versions' do
        expect do
          Version.create(:user => user, :vendor_spec => new_vendor_spec)
        end.should change(existing.versions, :count).by(1)
      end

      it "should not allow you upload an existing version" do
        version = Version.new(:user => user, :vendor_spec => existing_vendor_spec)
    ...
    context "with a new vendor" do
      ...
      let!(:version) { Version.create(:user => user, :vendor_spec => existing_vendor_spec) }
      ...
      it "should set the github link for the vendor" do
        version.vendor.github.should == "https://github.com/keithpitt/DKBenchmark"
      end
